...
function buildForm(form) {
  ...
  const items = [];
  bank.spelling.forEach((word, index) => items.push(item(form, items.length + 1, { part: 'A', category: 'spelling_dictation', primarySkill: index < 4 ? 'SP.patterns' : 'SP.wordparts', prompt: `Type the word you hear: ${word}.`, responseType: 'text', evaluator: 'spelling', acceptedAnswers: [word], explanation: 'Scored from the first typed spelling; the release version will play reviewed audio.' })));
  ...
  bank.listening.forEach(([first, second], index) => items.push(item(form, items.length + 1, { part: 'A', category: 'listening', primarySkill: 'PR.discrimination', prompt: `Listen to reviewed recording ${form}${index + 1} and choose whether you hear “${first}” or “${second}”.`, responseType: 'choice', evaluator: 'human_rubric', rubric: { status: 'blocked_until_reviewed_audio', options: [first, second] }, explanation: 'This draft cannot be scored until its contrasting audio is recorded and independently checked.' })));
