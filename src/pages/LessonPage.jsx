...
function Question({ item, onAnswer, busy, orderSeed }) {
  const [answer, setAnswer] = useState('');
  // Options are shown in a seed-derived order, so a patterned answer key cannot be read off the
  // position. `choice.id` still carries the answer, so nothing about scoring changes.
  ...
  return <form onSubmit={(event) => { event.preventDefault(); if (answer) onAnswer(answer); }}>
    <p>{item.prompt}</p>
    {choices.map((choice) => <label className={styles.choice} key={choice.id}><input type="radio" name={item.id} value={choice.id} checked={answer === choice.id} disabled={busy} onChange={() => setAnswer(choice.id)} /> {choice.text}</label>)}
...
export default function LessonPage() {
  ...
  // How many times this learner has already finished this lesson. Read once on mount so the served
  // questions cannot change underneath a sitting that is already in progress; the next mount picks up
  // the new count. Records written before this counter existed have no `completions`, and a recorded
  // completion without one counts as the first.
  const [sitting] = useState(() => {
    const record = readJson(`spelling-lesson-complete:${activeProfileId}:${sessionId}`, null);
    if (!record) return 0;
    return Number.isFinite(record.completions) ? record.completions : 1;
  });
  const baseLesson = lessonBySessionId(sessionId);
  const lesson = useMemo(() => lessonForSitting(baseLesson, sitting), [baseLesson, sitting]);
  ...
  const answer = async (response, metadata = {}) => {
    ...
    try {
      ...
      const nextState = { ...submitLessonResult(submissionState, evaluation.correct, { ...metadata, pending: evaluation.status === 'pending' }), evidenceIds: [...(submissionState.evidenceIds || []), attempt.attemptId] };
  ...
  const finish = (reflection) => {
    ...
    setState(completed);
    // `completions` advances the practice window for the next sitting. It only ever increases, so an
    // older record without the field is read as one completion rather than resetting the rotation.
    writeJson(`spelling-lesson-complete:${activeProfileId}:${sessionId}`, { sessionId, contentVersion: lesson.version, episodeId, completedAt: new Date().toISOString(), evidenceIds: completed.evidenceIds || [], reflection, completions: sitting + 1, ...(completed.reflectionNote ? { reflectionNote: completed.reflectionNote } : {}) });
  };
