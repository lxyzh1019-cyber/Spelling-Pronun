...
import { DEFAULT_LESSON_MINUTES, LESSON_MINUTE_OPTIONS, acceptWorkedSolution, completeReflection, completedLessonTasks, continueLesson, createLessonState, currentLessonItem, lessonResumeRecap, normalizeLessonMinutes, startLesson, submitLessonResult } from '../learning/lessonFlow';
...
export default function LessonPage() {
  ...
  const answer = async (response, metadata = {}) => {
    ...
    try {
      const { attempt, evaluation } = await submitAttempt(item, response, { sessionId, unseen: isTransfer, evidenceType: metadata.technicalFailure ? 'technical_failure' : metadata.omitted ? 'omission' : assisted ? 'assisted_repair' : isTransfer ? 'independent_transfer' : 'independent_choice', helped: assisted, ...metadata });
  ...
  return <div className={styles.page}>
    ...
    <section className={styles.card}>
      ...
      {state.stage === 'feedback' && <><div className={styles.feedback}><h2>{state.lastResult.technicalFailure ? 'Deferred: audio or microphone problem' : state.lastResult.correct ? 'Correct' : state.lastResult.omitted ? 'Not answered yet' : 'Not yet'}</h2><p>{state.lastResult.technicalFailure ? 'This was recorded as a technical issue, not a wrong answer. It is deferred and can be retried in a later review.' : state.lastResult.omitted ? 'This item remains unresolved. Use the guided repair before moving on.' : item.explanation}</p></div><button className={styles.primary} disabled={!writable} onClick={() => { if (canWrite()) setState(continueLesson(state, lesson.practice.length, lesson.transfer.length)); }}>{state.lastResult.correct || state.lastResult.technicalFailure ? 'Continue' : state.retryCount >= 2 ? 'See worked solution' : 'Repair this answer'}</button></>}
