...
function Question({ item, onAnswer }) {
  ...
  return <form onSubmit={(event) => { event.preventDefault(); if (answer) onAnswer(answer); }}>
    ...
    <button className={styles.primary} type="submit" disabled={!answer}>Save this answer</button>
  </form>;
}
...
export default function LessonPage() {
  ...
  const answer = async (response) => {
    ...
    const { attempt, evaluation } = await submitAttempt(item, response, { sessionId, unseen: isTransfer, evidenceType: assisted ? 'assisted_repair' : isTransfer ? 'independent_transfer' : 'independent_choice', helped: assisted });
    setState((current) => ({ ...submitLessonResult(current, evaluation.correct), evidenceIds: [...(current.evidenceIds || []), attempt.attemptId] }));
    setHelped(false);
  };
  ...
  return <div className={styles.page}>
    ...
    <section className={styles.card}>
      ...
      {['attempt', 'repair', 'transfer'].includes(state.stage) && <><h2>{state.stage === 'repair' ? 'Guided repair' : state.stage === 'transfer' ? `Unseen transfer ${state.transferIndex + 1} of ${lesson.transfer.length}` : `Independent question ${state.practiceIndex + 1} of ${lesson.practice.length}`}</h2>{state.stage === 'repair' && <p>Your first answer is preserved. This correction is recorded separately as assisted.</p>}<Question key={`${state.stage}-${item.id}-${state.retryCount}`} item={item} onAnswer={answer} />{!helped && <button className={styles.secondary} onClick={() => setHelped(true)}>Show help</button>}{helped && <div className={styles.feedback}><strong>Help used</strong>{item.helpSteps.map((step) => <p key={step}>{step}</p>)}<p>This response will be excluded from independent mastery evidence.</p></div>}</>}
      {state.stage === 'feedback' && <><div className={styles.feedback}><h2>{state.lastResult.correct ? 'Correct' : 'Not yet'}</h2><p>{item.explanation}</p></div><button className={styles.primary} onClick={() => setState(continueLesson(state, lesson.practice.length, lesson.transfer.length))}>{state.lastResult.correct ? 'Continue' : state.retryCount >= 2 ? 'See worked solution' : 'Repair this answer'}</button></>}
