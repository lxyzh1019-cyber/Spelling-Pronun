...
export default function AssessmentRunner() {
  ...
  const saveResult = async (response, metadata = {}) => {
    ...
    try {
      ...
      if (stateOwnerKey.current === submissionOwnerKey && canWrite()) {
        ...
        // learner gets a self-check against the stated rubric; it is recorded as self-report and is
        // never scored or counted as independent evidence.
        if (item.evaluator === 'human_rubric' && item.responseType === 'text' && !metadata.omitted && !metadata.technicalFailure) {
          setSelfCheck({ item, response, next: nextState });
        } else {
          setState(nextState);
  ...
  const finishSelfCheck = () => {
    if (!selfCheck) return;
    setState(selfCheck.next);
    setSelfCheck(null);
    setAnswer(''); setHelped(false); setAudioMessage('');
  };

  const takeOverHere = () => {
    setAnswer(''); setHelped(false); setAudioMessage(''); setSubmitting(false); setSelfCheck(null); submittingRef.current = false;
    takeOverSession();
  };
