...
export function buildContentManifest({ skills = [], packs = [], assessmentForms = [], episodes = [] }) {
  ...
  const counts = {
    ...
    integratedObjects: packs.flatMap((pack) => pack.items).filter((item) => item.integrationStatus === 'integrated').length,
    ...
    integratedAssessmentPrompts: assessmentForms.flatMap((form) => form.items).filter((item) => item.integrationStatus === 'integrated').length,
