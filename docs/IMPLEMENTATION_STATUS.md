...
| DATA-01 | Codex | R2 | verified locally | evaluator, mastery, scheduling, version-pinned delayed-review runner, revision-guarded IndexedDB session snapshots, attempt outbox, and learner-scoped local bootstrap | Recovery tests prove corrupt/missing local state restores only a matching learner/mode/content version; an isolated production-browser harness seeded question 3 only in IndexedDB, then independently verified both a missing and malformed localStorage mirror restored question 3 and rebuilt a valid mirror at revision 10; older IndexedDB revisions cannot replace newer snapshots; browser also verified exact lesson and assessment resume after route interruption and confirmed review bootstrap; draft content remains excluded from mastery | Connect authenticated remote sync after Firebase setup; exercise the review runner after C0 is independently reviewed |
...
| R1-G2 regressions | verified locally | Current `npm test`: 86/86; browser restart, skips, daily, crossword, learner ownership, reload checked |
...
| Content release validator | verified | Manifest separately counts challenged, reviewed, integrated, and released inventory and proves C0 96/68/2 counts without claiming release readiness; validator rejects unknown tasks/sources/skills, broken audio, reviewed or released synthetic audio, unsafe recording evaluators, malformed role distributions, integration without review, and release without completed review/integration |
