...
| DATA-01 | Codex | R2 | integrated | evaluator, mastery, scheduling, version-pinned delayed-review runner, session engine, IndexedDB outbox, and learner-scoped local store | Focused engine/validator/review-queue tests plus production build; browser confirmed draft content is refused entry to the reviewed mastery queue | Connect authenticated remote sync after Firebase setup; exercise the runner after C0 is independently reviewed |
...
| EDU-03 | Codex | R2 | integrated draft preview | `src/components/RecordingAnswer.jsx`, `src/utils/recording.js`, `src/utils/speech.js`, IndexedDB `recordings` store | Capability/MIME/cancellation tests; assessment recording UI; recording and speech stop on item, route, or learner change; explicit permission/technical failure reasons; blobs remain pending human review | Run a real iPad microphone, playback, persistence, and human-review test |
...
| R2 engine build | verified locally | `npm run build`: 117 modules transformed; lazy learner routes emitted |
...
| R2 full assessment runner | verified draft preview | Both 34-item forms integrated; browser verified synthetic audio control, explicit pause/exact resume, rapid-submit protection, learner-isolated switching, and 20-item Part A → 14-item Part B transition; the current 43-test suite and production build pass |
| R2 delayed-review runner | integrated / content blocked | Pure tests verify reviewed-only admission, four-item cap, stable item/version pinning, feedback preservation, machine-evaluable routing, and completion; browser confirms unreviewed C0 drafts cannot enter the mastery queue; 43 tests and production build pass | Independently review C0, then browser-test correct, helped, omitted, revealed-repair, pause/resume, and schedule advancement with released items |
