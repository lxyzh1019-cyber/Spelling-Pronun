...
| EDU-02 | Codex | R2 | integrated draft preview; 68/68 prompts challenged | Full 34-item `/assessment/:sessionId` Form A/B runner, `/progress`, `/review`, `/parent`, `assessmentReport.js`, `reviews.assessment.c0.json` | Browser showed 1/34 → reload at 2/34 and transition from Part A to Part B at 21/34; pause/resume, duplicate-submit protection, and learner isolation were exercised; every generated prompt now has an item-level challenge result; lesson-answer overlap, form length mismatch, ambiguous invented-word splits, placeholder feedback, underspecified speaking/writing rubrics, and miscounted editing targets were corrected and regression-tested | Complete educational/source review and replace synthetic audio with independently reviewed human audio before placement use |
...
| R1-G2 regressions | verified locally | Current `npm test`: 79/79; browser restart, skips, daily, crossword, learner ownership, reload checked |
...
| R2 C0 inventory | lesson/story reviewed; assessment challenge complete | Exact counts exist: 68 assessment prompts, four 24-object packs, and two story episodes. All 166 C0 objects/episodes completed recorded full-coverage challenge passes; all 96 lesson objects and both story episodes also completed separate educational/source reviews. Everything remains blocked from release/mastery pending assessment review, reviewed audio, integration, and learner testing |
...
| R2 full assessment runner | verified challenged preview | Both 34-item forms integrated and fully challenged; browser verified synthetic audio control, explicit pause/exact resume, rapid-submit protection, learner-isolated switching, speaking technical-failure continuation, exact route-interruption resume, and 20-item Part A → 14-item Part B transition; the current 79-test suite and production build pass |
| R2 delayed-review runner | integrated / content blocked | Pure tests verify reviewed-and-released-only admission, four-item cap, stable item/version pinning, feedback preservation, machine-evaluable routing, and completion; reviewed but unreleased lesson content is explicitly excluded, as is challenge-only content; browser confirms C0 preview content cannot enter the mastery queue and its durable session bootstrap is error-free; 79 tests and production build pass | Complete integration and learner testing, then browser-test correct, helped, omitted, revealed-repair, pause/resume, and schedule advancement with explicitly released items |
...
|---|---:|---:|---:|---:|---:|---:|
| C0 assessment A/B | 68 prompts | 68 draft | 68 | 0 | 0 | 0 |
...
- Shared family identity, assessment educational review, full assessment release, additional sourced story chapters, pronunciation recording, and family pilot remain R2/R3 work.
