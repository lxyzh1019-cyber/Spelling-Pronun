...
| EDU-01 | Codex | R2 | lesson packs and story educational/source reviewed; not released | `/case`, `lessonCatalog.js`, `lessonFlow.js`, `/lesson/:sessionId`, `reviews.c0.json`, `reviews.educational.c0.json`, `reviews.story.c0.json`, `reviews.educational.story.c0.json` | Browser completed the C0 spelling and capitals lessons; two misses produced a worked solution; `I don’t know yet` remained unresolved and entered guided repair; pause/back resumed that exact repair; all 96 lesson objects and both story episodes completed challenge and separate educational/source review with explicit object/episode coverage and source mappings; reviewed content remains excluded from release/mastery | Integrate and learner-test the lesson packs and story before release |
| EDU-02 | Codex | R2 | 68/68 challenged; 28/68 educational/source reviewed; not released | Full 34-item `/assessment/:sessionId` Form A/B runner, `/progress`, `/review`, `/parent`, `assessmentReport.js`, `reviews.assessment.c0.json`, `reviews.educational.assessment.c0.json` | Browser showed 1/34 → reload at 2/34 and transition from Part A to Part B at 21/34; pause/resume, duplicate-submit protection, and learner isolation were exercised; every prompt has an item-level challenge result; all 28 Part-B prompts have explicit educational/source-review coverage; all 40 Part-A prompts remain challenge-only with exact audio or specialist-review blockers; nineteen assessment discrepancies were corrected and recorded | Replace synthetic audio with independently reviewed human audio and complete specialist phonics/pronunciation review before placement use |
...
| R1-G2 regressions | verified locally | Current `npm test`: 82/82; browser restart, skips, daily, crossword, learner ownership, reload checked |
...
| R2 C0 inventory | lesson/story reviewed; assessment partially reviewed | Exact counts exist: 68 assessment prompts, four 24-object packs, and two story episodes. All 166 C0 objects/episodes completed recorded full-coverage challenge passes; all 96 lesson objects, both story episodes, and 28 Part-B assessment prompts also completed separate educational/source reviews. All remain blocked from release/mastery pending Part-A audio/specialist review, integration, and learner testing |
| Content release validator | verified | Manifest separately counts challenged, reviewed, and released inventory and proves C0 96/68/2 counts without claiming release readiness; validator rejects unknown tasks/sources/skills, broken audio, reviewed or released synthetic audio, unsafe recording evaluators, malformed role distributions, and release without completed author/educational review |
...
| R2 full assessment runner | verified partially reviewed preview | Both 34-item forms are integrated and fully challenged; 28 Part-B prompts are educational/source reviewed while 40 Part-A prompts retain exact blockers; browser verified synthetic audio control, explicit pause/exact resume, rapid-submit protection, learner-isolated switching, speaking technical-failure continuation, exact route-interruption resume, and 20-item Part A → 14-item Part B transition; the current 82-test suite and production build pass |
| R2 delayed-review runner | integrated / content blocked | Pure tests verify reviewed-and-released-only admission, four-item cap, stable item/version pinning, feedback preservation, machine-evaluable routing, and completion; reviewed but unreleased content is explicitly excluded, as is challenge-only content; browser confirms C0 preview content cannot enter the mastery queue and its durable session bootstrap is error-free; 82 tests and production build pass | Complete integration and learner testing, then browser-test correct, helped, omitted, revealed-repair, pause/resume, and schedule advancement with explicitly released items |
...
|---|---:|---:|---:|---:|---:|---:|
| C0 assessment A/B | 68 prompts | 68 draft | 68 | 28 | 0 | 0 |
| C0 four pilot packs | 96 objects | 96 draft | 96 | 96 | 0 | 0 |
| C0 chapter 1 | 2 episodes | 2 draft | 2 | 2 | 0 | 0 |
...
- IndexedDB session snapshots and the attempt outbox are integrated locally; direct browser recovery after deliberately deleting/corrupting the localStorage mirror still needs a shared-browser harness, while remote reconciliation, C0 integration/learner testing, pronunciation capture, and the family pilot remain incomplete.
